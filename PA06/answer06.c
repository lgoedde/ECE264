...
struct Image * loadImage(const char* filename)
{
  ...
  int return_val;
  return_val = fread(&hdr, sizeof(struct ImageHeader), 1, fptr);
  if(return_val != 1)
    {
      fclose(fptr);
      return NULL;
  ...
  return_val = fread(img->comment, sizeof(char), hdr.comment_len, fptr);
  if(return_val != hdr.comment_len)
    {
      fclose(fptr);
      freeImage(img);
      return NULL;
    }
  return_val = fread(img->data, sizeof(uint8_t), (hdr.width * hdr.height), fptr);
  if(return_val != (hdr.width*hdr.height))
    {
      fclose(fptr);
      freeImage(img);
      return NULL;
  ...
  return_val = fread(img->data, sizeof(uint8_t), 1, fptr);
